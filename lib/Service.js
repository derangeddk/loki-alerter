...
const Service = function Service({ config, log }) {
    const connections = [];
            ...
            config.alerts.forEach((alert) => {
                ...
                ws.on('message', (data) => {
                    ...
                    streams.forEach((stream) => {
                        ...
                        if (conditionTriggers.length) dispatch(config.receivers, log, alert, namespace, logLine, conditionTriggers);
                    });
                });
            });
        },
...
async function dispatch(receivers, log, alert, namespace, logLine, conditionTriggers) {
